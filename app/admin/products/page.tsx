...
import {
  Plus, Save, Loader2, Package, Tag, ChevronDown,
  ChevronRight, ToggleLeft, ToggleRight, Pencil, X, Database
} from 'lucide-react'
...
export default function ProductsAdminPage() {
  ...
  useEffect(() => {
    const unsub1 = productsService.subscribeAll(setProducts)
    const unsub2 = categoriesService.subscribeAll(setCategories)
    return () => { unsub1(); unsub2() }
  }, [])

  const allCategories: ProductCategory[] = categories.length > 0
    ? categories
    : DOUGH_CATEGORIES.map((c, i) => ({ id: c.id, label: c.label, color: c.color, sortOrder: i * 10, active: true }))
  ...
  // Fall back to static products if Firestore is empty
  const staticByCategory = STATIC_PRODUCTS.reduce((acc, p) => {
    if (!acc[p.category]) acc[p.category] = []
    acc[p.category].push(p)
    return acc
  }, {} as Record<string, Product[]>)
  ...
  const getCatProducts = (catId: string) =>
    products.length > 0 ? (productsByCategory[catId] || []) : (staticByCategory[catId] || [])
  ...
  const handleCreateProduct = async () => {
    ...
    try {
      await productsService.create({
        ...
        active: true,
        sortOrder: (getCatProducts(newProduct.category!).length + 1) * 10,
      })
  ...
  const handleCreateCategory = async () => {
    ...
    try {
      await categoriesService.create({
        ...
        sortOrder: (allCategories.length + 1) * 10,
        active: true,
      })
  ...
  const handleSeedDatabase = async () => {
    ...
    try {
      ...
      toast.success('All products seeded to database!')
  ...
  return (
    <AppShell>
      <div className="max-w-4xl mx-auto">
        ...
        {/* New Category Form */}
        {newCategory && (
          <div className="card p-5 mb-4 border-sage-400/40 animate-fade-in">
            <h3 className="font-display text-base mb-3 text-bark-900">New Product Category</h3>
            <div className="grid grid-cols-3 gap-3">
              <div className="col-span-2">
                <label className="label">Category Name</label>
                <input className="input" placeholder="e.g. Sourdough, Spelt, Einkorn..."
        ...
        {/* New Product Form */}
        {newProduct && (
          <div className="card p-5 mb-4 border-wheat-400/40 animate-fade-in">
              ...
              <div key={cat.id} className="card overflow-hidden">
                ...
                <div className="flex items-center gap-3 px-4 py-3 bg-cream-100">
                  <button onClick={() => toggleCat(cat.id)} className="flex items-center gap-3 flex-1 text-left">
                    ...
                    {isExpanded ? <ChevronDown className="w-4 h-4 text-bark-800/40 ml-auto" /> : <ChevronRight className="w-4 h-4 text-bark-800/40 ml-auto" />}
                  </button>
                ...
                {isExpanded && (
                  <div className="bg-white border-t border-wheat-400/20">
                    ...
                    {catProducts.map(p => (
                      <div key={p.id} className={`flex items-center gap-3 px-6 py-2.5 border-b border-wheat-400/10 last:border-0 ${!p.active ? 'opacity-50' : ''}`}>
                        {editingProduct?.id === p.id ? (
                          <div className="flex items-center gap-2 flex-1">
                            <input className="input flex-1 py-1 text-sm"
