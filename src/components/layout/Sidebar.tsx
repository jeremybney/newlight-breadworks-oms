...
const NAV_ITEMS = [
  { href: '/orders',     label: 'Orders',       icon: ClipboardList, roles: ['admin', 'staff', 'baker'] },
        ...
        {visibleItems.map(({ href, label, icon: Icon }) => {
          ...
          return (
            <Link
              ...
              href={href}
              className={`
                flex items-center gap-3 px-3 py-2.5 rounded text-sm font-body
                transition-all duration-150 group relative
                ${active
                  ? 'bg-wheat-500 text-cream-50'
                  : 'text-cream-200/70 hover:text-cream-50 hover:bg-cream-50/10'
                }
              `}
            >
