...
import { Customer, CustomerType, DOUGH_CATEGORIES } from '@/types'
...
export default function AdminPage() {
  ...
  const setPrice = (productId: string, price: string) => {
    setEditData(prev => ({
      ...prev,
      pricing: { ...prev.pricing, [productId]: parseFloat(price) || 0 }
    }))
  }
  ...
  const setSlicing = (productId: string, slicing: string) => {
    setEditData(prev => ({
      ...prev,
      slicing: { ...prev.slicing, [productId]: slicing }
    }))
  }
  ...
  return (
    <AppShell>
      <div className="flex h-[calc(100vh-4rem)] gap-5 -m-8 p-0">

        {/* LEFT — Customer List */}
        <div className="w-72 bg-cream-50 border-r border-wheat-400/20 flex flex-col flex-shrink-0 h-full">
              ...
              <button
                ...
                className={`w-full text-left px-4 py-3 border-b border-wheat-400/10 transition-colors ${
                  selectedCustomer?.id === c.id
                    ? 'bg-wheat-500/10 border-l-2 border-l-wheat-500'
                    : 'hover:bg-cream-100'
                }`}
              >
                ...
                <div className="text-xs font-mono text-bark-800/50 mt-0.5">
                  {c.route && <span className="mr-2">📍{c.route}</span>}
                  {c.type}
                </div>
              </button>
            ))}
          ...
          <div className="flex-1 flex flex-col overflow-hidden p-6">
            ...
            <div className="flex items-center justify-between mb-5">
              <h2 className="font-display text-xl text-bark-900">
                {isCreating ? 'New Customer' : editData.name}
              </h2>
            ...
            <div className="flex border-b border-wheat-400/20 mb-5">
              {[
                ...
                { id: 'slicing'   as Tab, label: 'Slicing', icon: Settings },
              ].map(({ id, label, icon: Icon }) => (
                <button
                  key={id}
                  onClick={() => setTab(id)}
                  className={`flex items-center gap-1.5 px-4 py-2.5 text-sm font-body border-b-2 transition-colors ${
                    tab === id
                      ? 'border-wheat-500 text-wheat-700 font-semibold'
                      : 'border-transparent text-bark-800/60 hover:text-bark-900'
                  }`}
                >
            ...
            <div className="flex-1 overflow-y-auto">
              ...
              {tab === 'customers' && (
                <div className="grid grid-cols-2 gap-x-6 gap-y-4 max-w-2xl animate-fade-in">
                  ...
                  <div>
                    ...
                    <input className="input" value={editData.route || ''} onChange={e => setEditData(p => ({...p, route: e.target.value}))} placeholder="e.g. B2, ND1" />
                  </div>
                  <div>
                    <label className="label">Code</label>
                    <input className="input font-mono" value={editData.code || ''} onChange={e => setEditData(p => ({...p, code: e.target.value}))} />
                  </div>
                  <div>
                    <label className="label">Packaging Type</label>
                    <input className="input font-mono" value={editData.packagingType || ''} onChange={e => setEditData(p => ({...p, packagingType: e.target.value}))} placeholder="e.g. ALLNP, CTS" />
                  </div>
                  <div>
                    <label className="label">Delivery Info</label>
                    <input className="input" value={editData.deliveryInfo || ''} onChange={e => setEditData(p => ({...p, deliveryInfo: e.target.value}))} placeholder="e.g. B2, ND1" />
                  </div>
                  <div>
                    <label className="label">Call Number</label>
                    <input className="input font-mono" value={editData.callNumber || ''} onChange={e => setEditData(p => ({...p, callNumber: e.target.value}))} />
                  </div>
                  <div>
                  ...
                  <div className="col-span-2">
                    <label className="label">Address / Delivery Notes</label>
                    <input className="input" value={editData.address || ''} onChange={e => setEditData(p => ({...p, address: e.target.value}))} placeholder="Delivery address and notes" />
                  </div>
                  <div className="col-span-2">
                    <label className="label">Internal Notes</label>
                    <textarea className="input resize-none h-16" value={editData.notes || ''} onChange={e => setEditData(p => ({...p, notes: e.target.value}))} />
                  </div>
                  <div className="col-span-2 flex items-center gap-3">
                    <button
                      onClick={() => setEditData(p => ({...p, active: !p.active}))}
              ...
              {tab === 'pricing' && (
                <div className="animate-fade-in max-w-2xl">
                  <p className="text-sm text-bark-800/60 mb-4">Set per-product prices for {editData.name}. Leave 0 for products this customer doesn't order.</p>
                              ...
                              <div key={p.id} className="flex items-center gap-3 px-4 py-2 border-b border-wheat-400/10 last:border-0">
                                ...
                                <div className="relative w-28">
                                  ...
                                  <input
                                    ...
                                    className="input pl-6 py-1 font-mono text-sm"
                                    placeholder="0.00"
                                  />
                                </div>
                              </div>
              ...
              {tab === 'slicing' && (
                <div className="animate-fade-in max-w-2xl">
                  <p className="text-sm text-bark-800/60 mb-4">Set default slicing preferences for {editData.name}. These will pre-fill on new orders.</p>
                  {DOUGH_CATEGORIES.map(cat => {
                    const catProducts = (productsByCategory[cat.id] || []).filter(p => p.active && p.canBeSliced)
                    if (!catProducts.length) return null
                            ...
                            {catProducts.map(p => (
                              <div key={p.id} className="flex items-center gap-3 px-4 py-2 border-b border-wheat-400/10 last:border-0">
                                ...
                                <select
                                  ...
                                  className="text-xs font-mono border border-wheat-400/30 rounded px-2 py-1 bg-white text-bark-900 focus:outline-none focus:border-wheat-500 w-32"
                                >
              ...
              <div className="fixed inset-0 bg-bark-900/50 flex items-center justify-center z-50">
                <div className="bg-cream-50 rounded-xl p-6 max-w-sm w-full mx-4 shadow-2xl">
                  ...
                  <p className="text-sm text-bark-800/70 mb-5">
                    {editData.name} will be hidden from order entry but historical orders are preserved.
                  </p>
