...
import { Customer, OrderItem, DoughCategory } from '@/types'
...
export default function OrdersPage() {
  ...
  useEffect(() => {
    customersService.subscribeAll(setCustomers)
  }, [])
  ...
  const handleSelectCustomer = (c: Customer) => {
    ...
    // Pre-fill slicing from customer preferences
    ...
    setItems(newItems)
    // Expand all categories that have customer slicing preferences
  ...
  const expandAll = () => setExpandedCategories(new Set(DOUGH_CATEGORIES.map(c => c.id)))
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (isRecurring) {
        ...
        // Fire Zapier webhook
        const order = await ordersService.getByDate(deliveryDate).then(orders =>
          orders.find(o => o.id === orderId)
        )
        if (order) sendToZapier(order, selectedCustomer)
    ...
    <AppShell>
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-3 gap-6">
          {/* LEFT COLUMN — Customer + Date + Settings */}
          <div className="col-span-1 space-y-4">
                  ...
                  <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-wheat-400/30 rounded shadow-lg z-50 max-h-60 overflow-y-auto">
                    {filteredCustomers.map(c => (
                      <button key={c.id} onClick={() => handleSelectCustomer(c)}
                        ...
                        <div className="text-xs text-bark-800/50 font-mono">{c.route} · {c.type}</div>
                      </button>
                    ))}
                  </div>
                )}
              ...
              {selectedCustomer && (
                <div className="mt-3 pt-3 border-t border-wheat-400/20 space-y-1.5 animate-fade-in">
                  <div className="flex justify-between text-xs">
                    <span className="text-bark-800/60">Route</span>
                    <span className="font-mono font-semibold text-wheat-600">{selectedCustomer.route}</span>
                  </div>
                  <div className="flex justify-between text-xs">
                    <span className="text-bark-800/60">Code</span>
                    <span className="font-mono font-semibold">{selectedCustomer.code}</span>
                  </div>
                  <div className="flex justify-between text-xs">
                    <span className="text-bark-800/60">Packaging</span>
                    <span className="font-mono font-semibold">{selectedCustomer.packagingType}</span>
                  </div>
                  <div className="flex justify-between text-xs">
                    <span className="text-bark-800/60">Delivery</span>
                    <span className="font-mono font-semibold">{selectedCustomer.deliveryInfo}</span>
                  </div>
                  {selectedCustomer.address && (
                    <div className="text-xs text-bark-800/60 mt-1 pt-1 border-t border-wheat-400/20">
                      {selectedCustomer.address}
                    </div>
                  )}
                </div>
                          ...
                          <button
                            ...
                            className={`px-2.5 py-1 text-xs font-mono rounded transition-colors ${
                              recurringDays.includes(day)
                                ? 'bg-wheat-500 text-cream-50'
                                : 'bg-cream-200 text-bark-800 hover:bg-cream-300'
                            }`}
                          >
                            {day}
                          </button>
            ...
            >
              {submitting ? (
                ...
                <><CheckCircle className="w-4 h-4" /> {isRecurring ? 'Set Up Recurring Order' : 'Submit Order'}</>
              )}
            </button>
          </div>

          {/* RIGHT COLUMNS — Product Selection */}
                    ...
                    return (
                      <div key={cat.id} className="border border-wheat-400/20 rounded overflow-hidden">
                        ...
                        >
                          ...
                          {catTotal > 0 && (
                            <span className="badge badge-wheat">{catTotal}</span>
                          )}
                          {isExpanded
                            ? <ChevronDown className="w-4 h-4 text-bark-800/40" />
                            : <ChevronRight className="w-4 h-4 text-bark-800/40" />
                          }
                        </button>
                  ...
                  {orderItems.map(item => (
                    <div key={item.productId} className="flex items-center justify-between text-sm py-1 border-b border-wheat-400/10 last:border-0">
                      <div className="flex items-center gap-2">
                        ...
                        {item.slicing && (
                          <span className="badge badge-sage text-xs">{item.slicing}</span>
                        )}
                      </div>
                      <span className="font-mono text-bark-800/70">
...
// ─── PRODUCT ROW ─────────────────────────────────────────────────────────────
function ProductRow({
  product, qty, slicing, onQtyChange, onSlicingChange, price
}: {
  ...
  return (
    <div className={`flex items-center gap-3 px-4 py-2.5 border-b border-wheat-400/10 last:border-0 transition-colors ${qty > 0 ? 'bg-wheat-400/5' : 'hover:bg-cream-50'}`}>
      ...
      {price !== undefined && (
        <span className="text-xs font-mono text-bark-800/40 w-12 text-right">${price.toFixed(2)}</span>
      )}
      ...
      {product.canBeSliced && (
        <select
          value={slicing}
          onChange={e => onSlicingChange(e.target.value)}
